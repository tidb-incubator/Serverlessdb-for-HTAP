...
import (
	...
	"github.com/tidb-incubator/Serverlessdb-for-HTAP/pkg/webhook/pod"
	"github.com/tidb-incubator/Serverlessdb-for-HTAP/pkg/webhook/strategy"
	...
	"github.com/tidb-incubator/Serverlessdb-for-HTAP/pkg/webhook/statefulset"
)
...
func (a *AdmissionHook) Validate(ar *admission.AdmissionRequest) *admission.AdmissionResponse {
	...
	kind := ar.Kind.Kind
	klog.Infof("receive validate request for %s[%s/%s]", kind, namespace, name)
