...
func (c *clientConn) writeResultsetForProxy( ctx context.Context,r *mysql.Resultset) error {
	sessionvar:=c.ctx.GetSessionVars()
	sta :=sessionvar.Status
	data := c.alloc.AllocWithLen(4, 1024)
	var err error
