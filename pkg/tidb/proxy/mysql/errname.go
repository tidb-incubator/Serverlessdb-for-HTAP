...
var MySQLErrName = map[uint16]string{
	...
	ER_Tidb_MUST_STOP:                               "This operation cannot be performed with a running Tidb; run STOP Tidb first",
	ER_Tidb_NOT_RUNNING:                             "This operation requires a running Tidb; configure Tidb and do START Tidb",
	ER_BAD_Tidb:                                     "The server is not configured as Tidb; fix in config file or with CHANGE MASTER TO",
	ER_MASTER_INFO:                                   "Could not initialize master info structure; more error messages can be found in the MySQL error log",
	ER_Tidb_THREAD:                                  "Could not create Tidb thread; check system resources",
	...
	ER_Tidb_IGNORED_TABLE:                           "Tidb SQL thread ignored the query because of replicate-*-table rules",
	...
	ER_Tidb_WAS_RUNNING:                             "Tidb is already running",
	ER_Tidb_WAS_NOT_RUNNING:                         "Tidb already has been stopped",
	...
	ER_Tidb_IGNORED_SSL_PARAMS:                      "SSL parameters in CHANGE MASTER are ignored because this MySQL Tidb was compiled without SSL support; they can be used later if MySQL Tidb with SSL is started",
	...
	ER_BAD_Tidb_UNTIL_COND:                          "Incorrect parameter or combination of parameters for START Tidb UNTIL",
	ER_MISSING_SKIP_Tidb:                            "It is recommended to use --skip-Tidb-start when doing step-by-step replication with START Tidb UNTIL; otherwise, you will get problems if you get an unexpected Tidb's mysqld restart",
	...
	ER_BINLOG_ROW_WRONG_TABLE_DEF:                    "Table definition on master and Tidb does not match: %s",
	ER_BINLOG_ROW_RBR_TO_SBR:                         "Tidb running with --log-Tidb-updates must use row-based binary logging to be able to replicate row-based binary log events",
	...
	ER_Tidb_INCIDENT:                                "The incident %s occured on the master. Message: %-.64s",
	...
	ER_Tidb_FATAL_ERROR:                             "Fatal error: %s",
	ER_Tidb_RELAY_LOG_READ_FAILURE:                  "Relay log read failure: %s",
	ER_Tidb_RELAY_LOG_WRITE_FAILURE:                 "Relay log write failure: %s",
	ER_Tidb_CREATE_EVENT_FAILURE:                    "Failed to create %s",
	ER_Tidb_MASTER_COM_FAILURE:                      "Master command %s failed: %s",
	...
	ER_NEVER_USED:                                    "Ambiguous Tidb modes combination. %s",
	...
	ER_Tidb_CORRUPT_EVENT:                                              "Corrupted replication event was detected",
	...
	ER_Tidb_HEARTBEAT_FAILURE:                                          "Unexpected master's heartbeat data: %s",
	ER_Tidb_HEARTBEAT_VALUE_OUT_OF_RANGE:                               "The requested value for the heartbeat period is either negative or exceeds the maximum allowed (%s seconds).",
	...
	ER_Tidb_IGNORE_SERVER_IDS:                                          "The requested server id %d clashes with the Tidb startup option --replicate-same-server-id",
	...
	ER_BINLOG_UNSAFE_SYSTEM_TABLE:                                       "The statement is unsafe because it uses the general log, slow query log, or performance_schema table(s). This is unsafe because system tables may differ on Tidbs.",
	...
	ER_BINLOG_UNSAFE_UDF:                                                "Statement is unsafe because it uses a UDF which may not return the same value on the Tidb.",
	ER_BINLOG_UNSAFE_SYSTEM_VARIABLE:                                    "Statement is unsafe because it uses a system variable that may have a different value on the Tidb.",
	ER_BINLOG_UNSAFE_SYSTEM_FUNCTION:                                    "Statement is unsafe because it uses a system function that may return a different value on the Tidb.",
	...
	ER_Tidb_CONVERSION_FAILED:                                          "Column %d of table '%-.192s.%-.192s' cannot be converted from type '%-.32s' to type '%-.32s'",
	ER_Tidb_CANT_CREATE_CONVERSION:                                     "Can't create conversion table for table '%-.192s.%-.192s'",
	...
	ER_Tidb_HEARTBEAT_VALUE_OUT_OF_RANGE_MIN:                           "The requested value for the heartbeat period is less than 1 millisecond. The value is reset to 0, meaning that heartbeating will effectively be disabled.",
	ER_Tidb_HEARTBEAT_VALUE_OUT_OF_RANGE_MAX:                           "The requested value for the heartbeat period exceeds the value of `Tidb_net_timeout' seconds. A sensible value for the period should be less than the timeout.",
	...
	ER_BINLOG_UNSAFE_INSERT_IGNORE_SELECT:                               "INSERT IGNORE... SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the Tidb.",
	ER_BINLOG_UNSAFE_INSERT_SELECT_UPDATE:                               "INSERT... SELECT... ON DUPLICATE KEY UPDATE is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are updated. This order cannot be predicted and may differ on master and the Tidb.",
	ER_BINLOG_UNSAFE_REPLACE_SELECT:                                     "REPLACE... SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are replaced. This order cannot be predicted and may differ on master and the Tidb.",
	ER_BINLOG_UNSAFE_CREATE_IGNORE_SELECT:                               "CREATE... IGNORE SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the Tidb.",
	ER_BINLOG_UNSAFE_CREATE_REPLACE_SELECT:                              "CREATE... REPLACE SELECT is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are replaced. This order cannot be predicted and may differ on master and the Tidb.",
	ER_BINLOG_UNSAFE_UPDATE_IGNORE:                                      "UPDATE IGNORE is unsafe because the order in which rows are updated determines which (if any) rows are ignored. This order cannot be predicted and may differ on master and the Tidb.",
	...
	ER_BINLOG_UNSAFE_WRITE_AUTOINC_SELECT:                               "Statements writing to a table with an auto-increment column after selecting from another table are unsafe because the order in which rows are retrieved determines what (if any) rows will be written. This order cannot be predicted and may differ on master and the Tidb.",
	ER_BINLOG_UNSAFE_CREATE_SELECT_AUTOINC:                              "CREATE TABLE... SELECT...  on a table with an auto-increment column is unsafe because the order in which rows are retrieved by the SELECT determines which (if any) rows are inserted. This order cannot be predicted and may differ on master and the Tidb.",
	...
	ER_MTS_FEATURE_IS_NOT_SUPPORTED:                                     "%s is not supported in multi-threaded Tidb mode. %s",
	...
	ER_SQLTHREAD_WITH_SECURE_Tidb:                                      "Setting authentication options is not possible when only the Tidb SQL Thread is being started.",
	...
	ER_BAD_Tidb_AUTO_POSITION:                                          "Parameters MASTER_LOG_FILE, MASTER_LOG_POS, RELAY_LOG_FILE and RELAY_LOG_POS cannot be set when MASTER_AUTO_POSITION is active.",
	...
	ER_GTID_MODE_REQUIRES_BINLOG:                                        "@@GLOBAL.GTID_MODE = ON or UPGRADE_STEP_1 or UPGRADE_STEP_2 requires --log-bin and --log-Tidb-updates.",
	...
	ER_MASTER_HAS_PURGED_REQUIRED_GTIDS:                                 "The Tidb is connecting using CHANGE MASTER TO MASTER_AUTO_POSITION = 1, but the master has purged binary logs containing GTIDs that the Tidb requires.",
	...
	ER_Tidb_CONFIGURATION:                                              "Tidb is not configured or failed to initialize properly. You must at least set --server-id to enable either a master or a Tidb. Additional error messages can be found in the MySQL error log.",
	...
	ER_MTS_CHANGE_MASTER_CANT_RUN_WITH_GAPS:                             "CHANGE MASTER cannot be executed when the Tidb was stopped with an error or killed in MTS mode. Consider using RESET Tidb or START Tidb UNTIL.",
	ER_MTS_RECOVERY_FAILURE:                                             "Cannot recover after Tidb errored out in parallel execution mode. Additional error messages can be found in the MySQL error log.",
	...
	ER_Tidb_SILENT_RETRY_TRANSACTION:                                   "Tidb must silently retry current transaction",
	...
	ER_GTID_NEXT_TYPE_UNDEFINED_GROUP:                                   "When @@SESSION.GTID_NEXT is set to a GTID, you must explicitly set it again after a COMMIT or ROLLBACK. If you see this error message in the Tidb SQL thread, it means that a table in the current transaction is transactional on the master and non-transactional on the Tidb. In a client connection, it means that you executed SET @@SESSION.GTID_NEXT before a transaction and forgot to set @@SESSION.GTID_NEXT to a different identifier or to 'AUTOMATIC' after COMMIT or ROLLBACK. Current @@SESSION.GTID_NEXT is '%s'.",
	...
	ER_SQL_Tidb_SKIP_COUNTER_NOT_SETTABLE_IN_GTID_MODE:                 "sql_Tidb_skip_counter can not be set when the server is running with @@GLOBAL.GTID_MODE = ON. Instead, for each transaction that you want to skip, generate an empty transaction with the same GTID as the transaction",
